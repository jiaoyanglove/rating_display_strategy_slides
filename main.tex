...
\textbf{电商平台的核心挑战}
\begin{itemize}
    \item 消费者面临\pos{信息不对称}问题
    \item 在线评论成为\HL{关键信息来源}
    \item 平台如何设计评论展示系统？
\end{itemize}
...
\textbf{典型场景：垂直差异化产品}
\begin{itemize}
    \item 同品牌不同型号（iPhone 15 vs 16）
    \item 同类别不同配置（基础款 vs 高端款）
\end{itemize}

\column{0.42\textwidth}
\begin{tikzpicture}[scale=0.85]
    % Platform
    \node[draw, rounded corners, fill=positive!15, minimum width=2.8cm, minimum height=0.9cm] (platform) at (0,2.8) {电商平台};

    % Products
    \node[draw, fill=negative!15, minimum width=1.1cm, minimum height=0.7cm, font=\small] (prodB) at (-0.9,1.4) {产品B};
    \node[draw, fill=emphasis!15, minimum width=1.1cm, minimum height=0.7cm, font=\small] (prodG) at (0.9,1.4) {产品G};

    % Reviews
    \node[draw, dashed, minimum width=2.6cm, minimum height=0.5cm, font=\small] (reviews) at (0,0.2) {评论数据};

    % Consumer
    \node[draw, circle, fill=cbPurple!20, minimum size=0.7cm, font=\small] (consumer) at (0,-0.9) {消费者};

    % Arrows
    \draw[->, thick] (platform) -- (prodB);
    \draw[->, thick] (platform) -- (prodG);
    \draw[->, thick] (prodB) -- (reviews);
    \draw[->, thick] (prodG) -- (reviews);
    \draw[->, thick, positive] (reviews) -- (consumer);
    \node[right, font=\footnotesize, positive] at (0.1,-0.35) {学习};
\end{tikzpicture}
\end{column}
\end{columns}
\end{frame}
...
\centering
\includegraphics[width=\textwidth]{figures/separate_example.png}
\end{alertblock}
\begin{itemize}
    \item 每个型号独立评分
...
\centering
\includegraphics[width=0.85\textwidth]{figures/integrate_example.png}
\end{exampleblock}
\begin{itemize}
    \item 整个系列合并评分
    \item 4.5星(674评)
